...
/**
 * This test class is testing the {@link DepartureRegister} class. The test class has both positive and negative
 * tests for the methods. The purpose of the class is to test the constructor and the set methods in the
 * {@code DepartureRegister} class. The tests mainly focuses on the input validation of the methods and in the constructor.
...
public class DepartureRegisterTest {
  ...
  class checkIfDepartureExistsTests {
    ...
    void shouldReturnFalseWhenDepartureDoesntExists() {
      ...
      assertFalse(depReg.checkIfDepartureExists(1, "L2", LocalTime.parse(("12:00"))));
    }
    @Test
    ...
    void shouldReturnTrueWhenDepartureExists() {
      DepartureRegister depReg = new DepartureRegister();
      depReg.newDeparture("12:00", "L1", 1, "Oslo", 1, 5);
      assertTrue(depReg.checkIfDepartureExists(1, "L1", LocalTime.parse(("12:00"))));
    }
    @Test
    @DisplayName("Positive test to still return true when line and departure time is null/empty.")
    void shouldReturnTrueWhenOnlyTrainNumberExists() {
      DepartureRegister depReg = new DepartureRegister();
      depReg.newDeparture("12:00", "L1", 1, "Oslo", 1, 5);
      assertTrue(depReg.checkIfDepartureExists(1, "", null));
    }
    @Test
    @DisplayName("Positive test to still return true when train number is not correct.")
    void shouldReturnTrueWhenLineAndTimeIsCorrect() {
      DepartureRegister depReg = new DepartureRegister();
      depReg.newDeparture("12:00", "L1", 1, "Oslo", 1, 0);
      assertTrue(depReg.checkIfDepartureExists(2, "L1", LocalTime.parse(("12:00"))));
    }
  }
  @Nested
  ...
  class minutesUntilDepartureTests {
    ...
    void shouldReturnMinutesUntilDepartureWhenDepartureExists() {
      ...
      assertEquals("30 min", depReg.minutesUntilDeparture(1));
    }
    @Test
    ...
    void shouldReturn0HoursAnd0Min() {
      ...
      assertEquals("0 min", depReg.minutesUntilDeparture(1));
    }
    @Test
    ...
    void shouldReturnNullWhenDepartureDoesNotExist() {
      ...
      DepartureRegister depReg = new DepartureRegister();
      depReg.newDeparture("10:30", "L1", 1, "Oslo", 1, 0);
      assertEquals("", depReg.minutesUntilDeparture(2));
    }
    ...
    void shouldReturnEmptyStringWhenCurrentTimeIsPastDepartureTime() {
      ...
      DepartureRegister depReg = new DepartureRegister();
      depReg.newDeparture("12:00", "L1", 1, "Oslo", 1, 5);
      assertEquals("", depReg.minutesUntilDeparture(1));
    }
  }
}